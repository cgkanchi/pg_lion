...
cp -r $P/src $P/Makefile $P/roaring_index.control $P/roaring_index--0.1.sql $B/; rm -f $B/src/*.o $B/src/*.bc $B/*.so; mkdir -p $B/test/sql $B/test/isolation
...
SCRATCH=$S T=$T ONLY="2b 2c" $P/bench/bench.sh > $P/results/${TAG}_bench_stdout.txt 2>&1
cp $S/bench_results.txt $P/results/${TAG}_queries.txt; cp $S/bench_plans.log $P/results/${TAG}_query_plans.log
