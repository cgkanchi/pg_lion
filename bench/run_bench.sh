...
cp -r $P/src $P/Makefile $P/roaring_index.control $P/roaring_index--0.1.sql $BUILD/
...
( cd $BUILD && make -s PG_CONFIG=$PREFIX/bin/pg_config && make -s PG_CONFIG=$PREFIX/bin/pg_config install ) && echo "extension installed into $PREFIX"
...
$PREFIX/bin/psql -X -q -f $P/bench/gen_roaring.sql | tee $P/results/06_roaring_index_build.log
SCRATCH=$(dirname $PREFIX) T=$T $P/bench/bench.sh | tee $P/results/06_bench_stdout.txt
cp $(dirname $PREFIX)/bench_results.txt $P/results/06_queries_with_roaring_index.txt 2>/dev/null || true
cp $(dirname $PREFIX)/bench_plans.log $P/results/06_query_plans_with_roaring_index.log 2>/dev/null || true
