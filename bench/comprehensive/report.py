...
Commit `{meta['commit']}`. Profile: **{meta['arguments'].get('profile', 'full')}**. Run status: **{meta.get('status','incomplete')}**. Started {meta['started_utc']}; completed {meta.get('completed_utc','not completed')}.
...
Focused document comparisons use GIN and roaring for text arrays and full-text search. The full profile also includes GiST text search; GiST has no text[] containment opclass here, so its array queries are fallbacks. Roaring scalar indexes do not provide ordered/range access; these cases remain in the focused profile to show that trade-off. Sequential execution is always the correctness reference, but is timed as a separate portfolio only when `seq` is selected. This suite does not claim coverage of geometric, vector, JSON, wildcard, or arbitrary extension indexes.
