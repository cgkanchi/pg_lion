-- Run against the benchmark cluster after `create extension pg_lion`.
\timing on
...
select format('drop index if exists fact_%1$s_roaring; create index fact_%1$s_roaring on fact using lion (%1$s);', c)
...
select c, s.* from unnest(array[:cols]) c, lateral lion_index_stats(('fact_' || c || '_roaring')::regclass) s;
