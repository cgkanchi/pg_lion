...
build_ext() { # $1 = container bits
  ...
  cp -r $P/src $P/Makefile $P/roaring_index.control $P/roaring_index--0.1.sql $B/; rm -f $B/src/*.o $B/src/*.bc $B/*.so
...
run_phases() { # $1 = tag
  $PREFIX/bin/psql -X -q -f $P/bench/gen_roaring.sql > $P/results/$1_roaring_index_build.log 2>&1
  SCRATCH=$S T=$T ONLY="2b 2c" $P/bench/bench.sh > $P/results/$1_bench_stdout.txt 2>&1
  cp $S/bench_results.txt $P/results/$1_queries.txt; cp $S/bench_plans.log $P/results/$1_query_plans.log
