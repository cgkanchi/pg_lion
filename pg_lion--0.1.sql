 ...
 * opclass is used instead (lion_fill_state()).  tsvector_ops stores text and
 * can name hashtext().
 * --------------------------------------------------------------------- */
...
COMMENT ON FUNCTION lion_index_stats(regclass) IS
	'shape of a lion index: pages, entries, containers by kind, sparse segments, NULL keys and key-less rows';
