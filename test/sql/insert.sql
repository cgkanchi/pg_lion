 ...
 * items have no slack.
 */
CREATE TABLE rbi_slack (i int4, k int4);
CREATE INDEX rbi_slack_k ON rbi_slack USING roaring (k) WITH (inline_limit = 64);
INSERT INTO rbi_slack SELECT i, i % 20 FROM generate_series(1, 20000) i;
SELECT array_containers > 0 AS has_arrays, slack_bytes > 0 AS has_slack,
  ...
  FROM roaring_index_stats('rbi_slack_k');
SELECT roaring_index_verify('rbi_slack_k', true);
SELECT rbi_cmp('rbi_slack', 'k = 7');

-- More inserts into the same container keys: they go into the slack, and the
-- posting sets stay exactly as correct.
CREATE TEMP TABLE rbi_slack_before AS
	SELECT container_bytes, container_pages FROM roaring_index_stats('rbi_slack_k');
INSERT INTO rbi_slack SELECT i, i % 20 FROM generate_series(20001, 22000) i;
SELECT ntids, slack_bytes > 0 AS has_slack,
	   slack_bytes <= 72 * (containers + sparse_segments) AS slack_within_bound
  FROM roaring_index_stats('rbi_slack_k');
SELECT (SELECT container_bytes FROM roaring_index_stats('rbi_slack_k')) >
	   (SELECT container_bytes FROM rbi_slack_before) AS items_grew;
SELECT roaring_index_verify('rbi_slack_k', true);
SELECT rbi_cmp('rbi_slack', 'k = 7');
SELECT rbi_cmp('rbi_slack', 'k = 0');

-- A bulk build writes items at their exact size: a bulk-built index of the
-- very same rows has no slack at all.
CREATE INDEX rbi_slack_b ON rbi_slack USING roaring (k) WITH (inline_limit = 64);
SELECT array_containers > 0 AS has_arrays, slack_bytes AS bulk_slack_bytes
  FROM roaring_index_stats('rbi_slack_b');
SELECT roaring_index_verify('rbi_slack_b', true);
 ...
 */
CREATE TABLE rbi_grow (i int4, k int4);
CREATE INDEX rbi_grow_k ON rbi_grow USING roaring (k);
SELECT nbuckets, max_bucket_pages FROM roaring_index_stats('rbi_grow_k');
INSERT INTO rbi_grow SELECT i, i FROM generate_series(1, 80000) i;
SELECT nbuckets, max_bucket_pages > 4 AS outgrown
  FROM roaring_index_stats('rbi_grow_k');
SELECT roaring_index_verify('rbi_grow_k', true);
SELECT rbi_cmp('rbi_grow', 'k = 12345');
CREATE TEMP TABLE rbi_grow_before AS
	SELECT nbuckets, max_bucket_pages FROM roaring_index_stats('rbi_grow_k');
REINDEX INDEX rbi_grow_k;
SELECT (SELECT nbuckets FROM roaring_index_stats('rbi_grow_k')) >
	   (SELECT nbuckets FROM rbi_grow_before) AS directory_grew,
	   (SELECT max_bucket_pages FROM roaring_index_stats('rbi_grow_k')) <
	   (SELECT max_bucket_pages FROM rbi_grow_before) AS chains_shortened;
SELECT roaring_index_verify('rbi_grow_k', true);
SELECT rbi_cmp('rbi_grow', 'k = 12345');
-- An explicit bucket count is never warned about: it is what its owner asked
-- for (rbi_spill_k above has buckets = 1 and 97 keys and says nothing).
CREATE INDEX rbi_grow_x ON rbi_grow USING roaring (k) WITH (buckets = 4);
INSERT INTO rbi_grow VALUES (80001, 80001);
SELECT nbuckets, max_bucket_pages > 4 AS outgrown
  FROM roaring_index_stats('rbi_grow_x');
DROP INDEX rbi_grow_x;
 ...
 */
CREATE TABLE rbi_relt (i int4, k int4);
INSERT INTO rbi_relt SELECT i, i FROM generate_series(1, 40000) i;
ANALYZE rbi_relt;
DELETE FROM rbi_relt WHERE i > 4000;
CREATE INDEX rbi_relt_k ON rbi_relt USING roaring (k);
CREATE INDEX rbi_relt_p ON rbi_relt USING roaring (k) WHERE i <= 2000;
SELECT (SELECT nbuckets FROM roaring_index_stats('rbi_relt_k')) >
	   (SELECT nbuckets FROM roaring_index_stats('rbi_relt_p'))
	   AS whole_table_sized_larger;
CREATE TEMP TABLE rbi_relt_before AS
	SELECT nbuckets FROM roaring_index_stats('rbi_relt_k');
-- once the statistics tell the truth, the same data wants a smaller directory
VACUUM (ANALYZE) rbi_relt;
REINDEX INDEX rbi_relt_k;
SELECT (SELECT nbuckets FROM roaring_index_stats('rbi_relt_k')) <
	   (SELECT nbuckets FROM rbi_relt_before) AS directory_shrank;
SELECT roaring_index_verify('rbi_relt_k', true);
SELECT roaring_index_verify('rbi_relt_p', true);
SELECT rbi_cmp('rbi_relt', 'k = 1234');

DROP TABLE rbi_ins, rbi_spill, rbi_dense, rbi_mid, rbi_inlmid, rbi_slack,
		   rbi_grow, rbi_relt;
